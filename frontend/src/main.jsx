...
function AgentSettings({config, close, saved}) {
  const [provider,setProvider]=useState(config?.activeProvider || 'cloudera');
  const current=()=>config?.providers?.find(item=>item.id===provider) || {};
  const [endpoint,setEndpoint]=useState(current().endpoint || '');
  const [models,setModels]=useState((current().models || []).join(', '));
  const [defaultModel,setDefaultModel]=useState(current().defaultModel || '');
  const [token,setToken]=useState(''); const [error,setError]=useState(''); const [busy,setBusy]=useState(false);
  const chooseProvider=value=>{const profile=config.providers.find(item=>item.id===value)||{};setProvider(value);setEndpoint(profile.endpoint||'');setModels((profile.models||[]).join(', '));setDefaultModel(profile.defaultModel||'');setToken('');setError('')};
  const submit=async event=>{event.preventDefault();setBusy(true);setError('');try{const catalog=models.split(',').map(x=>x.trim()).filter(Boolean);const updated=await request('/agent/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({provider,endpoint,token:token||null,models:catalog,default_model:defaultModel})});saved(updated);window.dispatchEvent(new Event('diagnostics-refresh'));close();}catch(err){setError(err.message)}finally{setBusy(false)}};
  const profile=current();
  return <div className="modal" role="presentation" onMouseDown={event=>event.target===event.currentTarget&&close()}><form className="agent-settings" onSubmit={submit} role="dialog" aria-modal="true" aria-labelledby="agent-settings-title">
    <header><div><p className="eyebrow">AGENTE DE GOBIERNO</p><h2 id="agent-settings-title">Configuración del agente</h2></div><button type="button" className="icon-button" onClick={close} aria-label="Cerrar"><X size={19}/></button></header>
    <p className="settings-copy">Selecciona AI Gateway/LiteLLM o Cloudera AI Inference. El secreto solo viaja al backend y nunca se devuelve a la interfaz ni se escribe en el log.</p>
    <div className="settings-grid">
      <label>Proveedor<select value={provider} onChange={e=>chooseProvider(e.target.value)}>{config.providers.map(item=><option key={item.id} value={item.id}>{item.label}</option>)}</select><small>{profile.authType}</small></label>
      <label>Endpoint<input type="url" value={endpoint} onChange={e=>setEndpoint(e.target.value)} placeholder="https://modelo.example/v1" required/></label>
      <label className="settings-wide">Modelos publicados<input value={models} onChange={e=>setModels(e.target.value)} placeholder="modelo-a, modelo-b" required/><small>Lista separada por comas.</small></label>
      <label>Modelo predeterminado<input value={defaultModel} onChange={e=>setDefaultModel(e.target.value)} required/></label>
      <label>Token<input type="password" value={token} onChange={e=>setToken(e.target.value)} autoComplete="new-password" placeholder={profile.tokenConfigured?'Configurado · dejar vacío para conservar':'CDP Token o API Token'}/><small>{profile.tokenConfigured?`Credencial activa: ${profile.tokenSource}.`:'No hay un secreto configurado.'}</small></label>
    </div>
    <div className="settings-notice"><ShieldCheck size={17}/>El cambio se mantiene durante la sesión del proceso CML. Para hacerlo persistente, configura las variables equivalentes del AMP.</div>
    {error&&<div className="login-error"><AlertTriangle size={17}/>{error}</div>}
    <footer><button type="button" className="secondary" onClick={close}>Cancelar</button><button type="submit" disabled={busy}>{busy?'Guardando…':'Aplicar configuración'}</button></footer>
  </form></div>;
}
...
function Configuration({config, close, saved}) {
  ...
  return <div className="modal"><form className="config-panel" onSubmit={submit}><header><div><h2>Configuración</h2><p>Conexiones de Ranger, auditoría y modelo LLM para esta sesión CML.</p></div><button type="button" onClick={close}>Cerrar</button></header>
    <fieldset><legend>Apache Ranger / Knox</legend>
      <label className="span-2"><FieldTitle help="URL base de Apache Ranger publicada por Knox o URL completa de access_audit. La aplicación detecta y normaliza ambos formatos sin duplicar la ruta REST." example="https://go01-aw-dl-gateway.go01-dem.ylcu-atmi.cloudera.site/service/xaudit/access_audit">URI de Ranger</FieldTitle><input value={form.ranger_url} onChange={e=>set('ranger_url',e.target.value)} required/></label>
...
function App(){
  ...
  const [provider,setProvider]=useState('cloudera'); const [models,setModels]=useState([]); const [model,setModel]=useState('');
  ...
  const applyAgentConfig=agent=>{const active=agent.providers.find(item=>item.id===agent.activeProvider)||agent.providers[0];setRuntimeConfig(current=>({...current,agent}));setProvider(active?.id||'cloudera');setModels(active?.models||[]);setModel(active?.defaultModel||active?.models?.[0]||'')};
  useEffect(()=>{if(auth)request('/config').then(config=>{setRuntimeConfig(config);applyAgentConfig(config.agent)}).catch(()=>{})},[auth]);
