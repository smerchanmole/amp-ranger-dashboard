...
function DiagnosticsBar() {
  ...
  return <aside className="connection-diagnostics" aria-label="Estado de conexiones">
    {Object.entries(labels).map(([key,label])=>{const state=services?.[key];const title=state?.ok?`Conectado en ${state.latencyMs} ms`:(state?.error||'Sin comprobar');return <span key={key} className={!state?'pending':state.ok?'ok':'ko'} title={title}><i/>{label}</span>})}
